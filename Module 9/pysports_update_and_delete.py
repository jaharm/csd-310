...
try:
    ...
    add_player = ("INSERT INTO player(first_name, last_name, team_id)"
                 "VALUES(%s, %s, %s)")
    player_data = ("Joe", "Bob", 1)
    cursor.execute(add_player, player_data)
    db.commit()
    show_players(cursor, "DISPLAYING PLAYERS AFTER INSERT")
    update_player = ("UPDATE player SET team_id = 2, first_name = 'Jane', last_name = 'Austen Book Writer' WHERE first_name = 'Joe'")
    cursor.execute(update_player)
    show_players(cursor, "DISPLAYING PLAYERS AFTER UPDATE")
    delete_player = ("DELETE FROM player WHERE first_name = 'Jane'")
    cursor.execute(delete_player)
    delete_player = ("DELETE FROM player WHERE first_name = 'Joe'")
    cursor.execute(delete_player)
